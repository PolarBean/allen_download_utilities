...
def downsample_image(image, source_pixel_size, target_pixel_size):
    downsampled_image = cv2.resize(image, (0,0), fx=source_pixel_size/target_pixel_size, fy=source_pixel_size/target_pixel_size)
...
def download_image(output_dir, animal_name, experiment_id, image_id, image_number, resolution=None, view=None):
    """
    ...
    if view == 'expression':
        ...
        for target_pixel_size in [10, 25]:
            downsampled_image = downsample_image(image_array, source_pixel_size, target_pixel_size)
            output_path = os.path.join(output_dir, animal_name, str(experiment_id), f"{target_pixel_size}um", f"{image_id}_s{image_number:04}.jpg")
